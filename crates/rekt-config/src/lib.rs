...
//!
//! [catalog]                      # reconciler cadence + cache refresh
//! reconcile_interval_ms    = 30000
//! invalidate_on_local_ddl  = true
//! ```
//!
//! As of PLAN-10 D8, table declarations no longer live in TOML — tables
//! are runtime objects, declared exclusively via the DDB `CreateTable`
//! wire API. See `docs/plan/PLAN-10-ddl-operations.md`.
...
/// Fully-resolved config: defaults applied, validation rules checked.
///
/// Tables are NOT declared in config — they're runtime objects mutated
/// via the DDB `CreateTable` / `DeleteTable` wire API (PLAN-10 D8).
#[derive(Debug, Clone)]
pub struct Config {
